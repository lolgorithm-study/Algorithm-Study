...
// 절대 값을 기준으로 최소 힙으로 정렬하지만, 같은 절대 값이 있을 경우 일반 값중에 작은 값으로 정렬한다.
let absSortHeap: (HeapData, HeapData) -> Bool = {
    if $0.absValue == $1.absValue {
        return $0.normalValue < $1.normalValue
    }
    return $0.absValue < $1.absValue
}
// 여기서 sort 는 heap 구조체의 sort T, T -> Bool 의 매개변수이다.
// 즉, HeapData 타입을 갖는 우선순위 큐를 생성했을 때 정렬 기준은 absSortHeap 으로 할 것인데,
// 이것을 orderCriteria 에 할당해놓고, 최소 우선순위 큐에 값이 들어오거나 나갈때 shiftUp, shiftDown 에서 우리가 정한 정렬 기준(absSortHeap)을 따르게 된다.
var minPriorityQueue = PriorityQueue<HeapData>(sort: absSortHeap)
...
for _ in 0..<N {
    ...
    if data == 0 {
        print(minPriorityQueue.dequeue()?.normalValue ?? 0)
    } else {
        minPriorityQueue.enqueue(element: HeapData(normalValue: data, absValue: abs(data)))
    }
}
