// BOJ_23757_아이들과 선물 상자.swift
// 모든 아이들이 실망하지 않고 각자 원하는 만큼 선물을 가져갈 수 있으면 1을, 그렇지 않으면 0을 출력한다.

let input = readLine()!.split(separator: " ").map { Int($0)! }
let N = input[0], M = input[1]

let giftBox = readLine()!.split(separator: " ").map { Int($0)! }
let desiredGift = readLine()!.split(separator: " ").map { Int($0)! }

var result = 1
var maxPQ = PriorityQueue<Int>(sort: >)

for i in giftBox {
    maxPQ.enqueue(element: i)
}

for index in desiredGift {
    let maxBox = maxPQ.dequeue()!
    // 가장 큰 선물박스가 아이들이 원하는 선물의 갯수보다 크다면, 선물을 가져간다.
    if maxBox >= index {
        let remainingBox = maxBox - index
        // 남은 선물이 0개가 아니라면 다시 우선순위 큐에 넣는다.
        if remainingBox != 0 {
            maxPQ.enqueue(element: maxBox - index)
        }
    } else {
        result = 0
        break
